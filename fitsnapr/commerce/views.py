...
def add_to_cart(request, id):
    ...
    order_qs = LeagueOrder.objects.filter(user_profile=user_profile, ordered=False)
    if order_qs.exists():
        order = order_qs[0]
...
def reduce_from_cart(request, id):
    user_profile = request.user.userprofile
    package = get_object_or_404(LeaguePackage, id=id)
    cart_qs = LeagueCart.objects.filter(user_profile=user_profile, league_package=package)
    if cart_qs.exists():
        cart = cart_qs[0]
        if cart.quantity > 1:
...
def remove_from_cart(request, id):
    ...
    if order_qs.exists():
        ...
        if order.league_packages.filter(league_package_id=package.id).exists():
            order_item = LeagueCart.objects.filter(league_package=package, user_profile=user_profile)[0]
...
class CartView(View):
    def get(self, request):
        order = LeagueCart.objects.filter(user_profile=request.user.userprofile, leagueorder__ordered=0)
